...
access_mcp_resource_tool = FunctionTool.from_defaults(
    ...
    description=(
        "Accesses and retrieves a specific resource (e.g., file content, image, structured data) "
        "from a designated MCP (Micro-Credentialing Provider) server. "
        "This tool enables the agent to fetch data or assets from external services that are "
        "exposed as resources via an MCP. It is crucial for retrieving information needed "
        "for tasks like reading documentation, viewing images, or fetching configuration data "
        "from connected platforms."
        "\\n\\nArgs:"
        "\\n- `server_name` (str): The name of the MCP server from which to access a resource. "
        "  This identifies the specific external service or platform (e.g., `GitHub_MCP`, `Jira_MCP`)."
        "\\n- `uri` (str): The URI (Uniform Resource Identifier) of the specific resource to access "
        "  on the MCP server. This is typically a path or endpoint that points to "
        "  data, files, or other assets (e.g., `/repos/my_project/README.md`, `/users/avatar.png`, `/api/v1/data.json`)."
        "\\n\\nReturns:"
        "\\n  A formatted string containing the content of the accessed resource, "
        "  or an error message if the access failed (e.g., resource not found, "
        "  permission issues, server error). If the resource is an image or other "
        "  non-textual content, a note will indicate its presence as the image "
        "  itself cannot be directly rendered in text output."
        "\\n\\n**Important Note on Implementation:** This Python tool provides a **stubbed implementation** "
        "for the `read_mcp_resource` function. In a real-world scenario, this stub would need to be replaced "
        "with actual Python client code that interacts with the specific MCP system via its API. "
        "The current stub simulates responses based on the `server_name` and `uri` and does not connect to any live external service."
        "\\n\\nExample Usage:"
        '\\n`{"tool_code": "access_mcp_resource(server_name=\\"GitHub_MCP\\", uri=\\"/repos/my_project/README.md\\")"}`'
        '\\n`{"tool_code": "access_mcp_resource(server_name=\\"Jira_MCP\\", uri=\\"/issues/PROJ-456/attachments/image.png\\")"}`'
        '\\n`{"tool_code": "access_mcp_resource(server_name=\\"Internal_Docs\\", uri=\\"/docs/api_spec.json\\")"}`'
    ),
