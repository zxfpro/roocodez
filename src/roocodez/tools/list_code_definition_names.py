...
list_code_definition_names_tool = FunctionTool.from_defaults(
    ...
    description=(
        "Lists the names of code definitions (e.g., functions, classes, exported variables) "
        "found within a specified file or directory. This tool helps understand the internal "
        "structure and available components of code files without reading their full content. "
        "It is useful for quickly identifying key functionalities or types defined in a codebase."
        "\n\n**Note on Accuracy:** This tool uses simplified parsing techniques (regex-based stubs "
        "in this Python version). While it aims to identify common definitions, it may not be "
        "as accurate or comprehensive as a dedicated AST parser (like Tree-sitter used in the "
        "original TypeScript version), especially for complex or less common syntax patterns. "
        "Use `read_file` for detailed content if needed."
        "\n\nArgs:"
        "\n- `path` (str): The relative path to the file or directory. "
        "  If a file (e.g., `src/utils.py`), it parses that file. "
        "  If a directory (e.g., `src/controllers/`), it attempts to parse "
        "  common code files (.py, .js, .ts, etc.) in its top level (non-recursively)."
        "\n\nReturns:"
        "\n  A formatted string listing the detected code definitions, or a message "
        "  indicating if no definitions were found, or if the path does not exist/is inaccessible."
        "\n\nExample Usage:"
        '\n`{"tool_code": "list_code_definition_names(path=\'src/agents/react_agent.py\')"`'
        '\n`{"tool_code": "list_code_definition_names(path=\'tests/\')"`'
    ),
