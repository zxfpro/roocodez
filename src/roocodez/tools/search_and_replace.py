...
search_and_replace_tool = FunctionTool.from_defaults(
    ...
    description=(
        "Performs search and replace operations on a specified file. "
        "This tool is highly versatile for modifying file content, "
        "from simple string replacements to complex regex-based transformations. "
        "It can be used to update variable names, refactor code, or fix patterns across a file."
        "\\n\\nArgs:"
        "\\n- `path` (str): The relative path to the file on which to operate. "
        "  Example: `path=\'src/config.json\'`."
        "\\n- `search` (str): The string or regex pattern to search for. "
        "  Example: `search=\'old_function_name\'` or `search=\'\\\\b(foo|bar)\\\\b\'`."
        "\\n- `replace` (str): The string to replace matches with. "
        "  Supports backreferences (`\\1`, `\\2`, etc.) if `use_regex` is true. "
        "  Example: `replace=\'new_function_name\'` or `replace=\'baz_\\1\'`."
        "\\n- `use_regex` (bool, optional): Set to `true` if `search` is a regular expression. "
        "  Defaults to `false` (literal string search). "
        "  Example: `use_regex=true`."
        "\\n- `ignore_case` (bool, optional): Set to `true` for case-insensitive search. "
        "  Defaults to `false` (case-sensitive). "
        "  Example: `ignore_case=true`."
        "\\n- `start_line` (int, optional): 1-based starting line number (inclusive) for the operation. "
        "  If provided, only lines from this number onwards are processed. "
        "  Example: `start_line=10`."
        "\\n- `end_line` (int, optional): 1-based ending line number (inclusive) for the operation. "
        "  If provided, only lines up to this number are processed. "
        "  Example: `end_line=25`."
        "\\n\\nReturns:"
        "\\n  A string message indicating the outcome: success, no changes made, or an error "
        "  message with details if the operation failed (e.g., file not found, "
        "  read/write permissions, invalid regex).\n\n"
        "\\n\\nExample Usage:"
        '\\n`{"tool_code": "search_and_replace(path=\\\'src/main.py\\\', search=\\\'old_var\\\', replace=\\\'new_var\\\')"`'
        '\\n`{"tool_code": "search_and_replace(path=\\\'config.js\\\', search=\\\'(\\\\w+)_KEY\\\', replace=\\\'\\\\1_SECRET\\\', use_regex=true, ignore_case=true)"}`'
        '\\n`{"tool_code": "search_and_replace(path=\\\'test.txt\\\', search=\\\'TODO\\\', replace=\\\'DONE\\\', start_line=5, end_line=10)"}`'
    ),
