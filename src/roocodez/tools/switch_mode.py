...
switch_mode_tool = FunctionTool.from_defaults(
    ...
    description=(
        ...
        "This tool allows the agent to adjust the environment's focus or configuration "
        "to better suit the current task (e.g., switching to a 'code_editing' mode "
        "when writing code, or 'documentation' mode when reading documentation). "
        "Each mode might enable different functionalities or prioritize certain views/interactions."
        "\\n\\nArgs:"
        "\\n- `mode_slug` (str): The unique identifier (slug) of the mode to switch to. "
        "  Available modes typically include: "
        f"{', '.join([f'`{s}`' for s in MODES_STUB.keys()])}. " # Dynamically list available modes
        "  Example: `mode_slug='code_editing'`."
        "\\n- `reason` (str, optional): An optional, brief explanation of why the mode switch "
        "  is necessary or beneficial. This helps the user understand the agent's intent. "
        "  Example: `reason='Need to focus on writing new code.'`."
        "\\n\\nReturns:"
        "\\n  A string message indicating whether the mode switch was successful, "
        "  if the specified mode is invalid, or if the environment is already "
        "  in the requested mode."
        "\\n\\n**Important Note on Implementation:** This Python tool provides a **stubbed implementation** "
        "for mode management. It simulates mode changes internally but does not directly "
        "alter a real external application's UI or global state. The agent is responsible "
        "for interpreting the success message and, if necessary, communicating this "
        "change to the user or orchestrating application-level mode changes externally."
        "\\n\\nExample Usage:"
        '\\n`{"tool_code": "switch_mode(mode_slug=\\\'code_editing\\\', reason=\\\'Starting a new coding task.\\\')"`}'
        '\\n`{"tool_code": "switch_mode(mode_slug=\\\'documentation\\\')"`}'
    ),
