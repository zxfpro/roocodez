...
list_files_tool = FunctionTool.from_defaults(
    ...
    description=(
        ...
        "Useful for understanding the file structure of the project, exploring "
        "directory contents, or verifying file existence. "
        "The tool returns a formatted string listing the items found."
        "\n\nArgs:"
        "\n- `path` (str, optional): The relative path to the directory to list files from. "
        "  If not provided, the current working directory of the agent will be used. "
        "  Example: `path='src/utils'`"
        "\n- `recursive` (bool, optional): If set to `true`, the tool will list files "
        "  and directories recursively in all subdirectories. Defaults to `false` "
        "  (only lists top-level items in the specified directory). "
        "  Example: `recursive=true`"
        "\n\nExample Usage:"
        '\n`{"tool_code": "list_files(path=\'src\', recursive=true)"}`'
        '\n`{"tool_code": "list_files()"}` (to list current directory non-recursively)'
    ),
