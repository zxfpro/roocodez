...
insert_content_tool = FunctionTool.from_defaults(
    ...
    description=(
        ...
        "This tool is used to add new lines or blocks of code/text into an existing "
        "file or to create a new file and insert content into it. "
        "It is useful for precise modifications like adding a new function, "
        "import statement, or configuration entry without affecting surrounding code."
        "\n\nArgs:"
        "\n- `path` (str): The relative path to the file where the content will be inserted. "
        "  If the file does not exist, it will be created."
        "  Example: `path='src/services/data_access.py'`."
        "\n- `line` (int): The 1-based line number before which to insert the content. "
        "  For example, `line=1` means insert at the very beginning of the file. "
        "  To append content to the end of the file, you can specify a line number "
        "  greater than the current number of lines (e.g., `len(file_lines) + 1`). "
        "  `line=0` will also be treated as inserting at the beginning."
        "  Example: `line=5`."
        "\n- `content` (str): The multi-line string content to be inserted. "
        "  Newlines within this string will be preserved."
        ...
        "\n\nReturns:"
        "\n  A string message indicating whether the content was successfully inserted, "
        "  or an error message with details if the operation failed (e.g., invalid path, "
        "  permission issues, or invalid line number)."
        "\n\nExample Usage:"
        '\n`{"tool_code": "insert_content(path=\'src/app.py\', line=1, content=\'\'\'import logging\n\'\'\')"`'
        '\n`{"tool_code": "insert_content(path=\'docs/README.md\', line=0, content=\'\'\'# Project Overview\n\n\'\'\')"`'
        '\n`{"tool_code": "insert_content(path=\'config.json\', line=999, content=\'\'\'\n  "new_setting": true\n\'\'\')"` (appending)'
    ),
