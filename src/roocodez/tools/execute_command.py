...
execute_command_tool = FunctionTool.from_defaults(
    ...
    description=(
        "Executes a shell command in the operating system's terminal. "
        "This tool is highly versatile and allows the agent to interact with the "
        "underlying file system, run build tools (e.g., `npm`, `yarn`, `pip`, `make`), "
        "inspect environment variables, or perform any operation that can be done via a shell command. "
        "The output of the command (stdout and stderr) is captured and returned. "
        "Use this tool to: "
        "\\n- List directory contents (`ls`, `dir`)."
        "\\n- Check file permissions or existence (`stat`, `test -f`)."
        "\\n- Run tests (`npm test`, `pytest`)."
        "\\n- Install dependencies (`npm install`, `pip install`)."
        "\\n- Compile code."
        "\\n- Interact with version control (`git status`, `git diff`)."
        "\\n\\nArgs:"
        "\\n- `command` (str): The full shell command string to execute. Example: `command=\'ls -l\'`."
        "\\n- `cwd` (str, optional): The relative path to the directory where the command should be executed. "
        "  If not provided, the command will be executed in the agent's current working directory. "
        "  Example: `cwd=\'./backend\'`."
        "\\n\\nReturns:"
        "\\n  A string containing the complete output from the executed command (stdout and stderr), "
        "  including its exit code. The output might be truncated if it's very long."
        "\\n\\n**Important Note on Execution:** This tool executes commands directly on the host system. "
        "Be mindful of the commands you generate. The execution is atomic; "
        "it runs to completion and returns the final output. It does not support real-time "
        "interactive sessions or long-running background processes."
        "\\n\\nExample Usage:"
        "\\n`{\"tool_code\": \"execute_command(command=\'git status\')\"}`"
        "\\n`{\"tool_code\": \"execute_command(command=\'npm install\', cwd=\'./frontend\')\"}`"
        "\\n`{\"tool_code\": \"execute_command(command=\'python -c \\\"import sys; print(sys.version)\\\"\')\"}`"
    ),
